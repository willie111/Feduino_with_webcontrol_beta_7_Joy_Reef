...
struct config_PHR                          // PH calcuim reactor
{
  byte PHRset;
  byte PHRoff;
  byte PHRalarm;
...
struct config_tpa                       // auto-water change
{
  byte temphora;
  byte tempminuto;
...
{
  byte k = 222;
  EEPROM.write(796, k);         // Verifica se hÃƒÂ¡ valores para os LEDs na EEPROM
  for (int i=1; i<97; i++)  
  {
...
// PosiÃƒÂ§ÃƒÂµes 635 atÃƒÂ© 687 estÃƒÂ£o livres
// 688 ponteiro para log de temperatura da ÃƒÂ¡gua.
// 689 ponteiro para log de ph do aquÃƒÂ¡rio.
// 690 ponteiro para log de ph do reator de cÃƒÂ¡lcio.
// 691 ponteiro para log de orp.
// 692 ponteiro para log de densidade.
// 693 ponteiro para a temperatura da ÃƒÂ¡gua.
...
//PosiÃƒÂ§ÃƒÂµes 709 a 710 disponÃƒÂ­veis.
